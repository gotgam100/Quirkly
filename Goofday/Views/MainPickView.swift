...
struct MainPickView: View {
    ...
    @State private var currentTask: GoofyTask?
    ...
    private var backgroundView: some View {
        ZStack {
            ...
            GeometryReader { geo in
                SparkleView(size: 20, color: .goofYellow).position(x: 40, y: 80)
                SparkleView(size: 14, color: .goofPink).position(x: geo.size.width - 50, y: 120)
                SparkleView(size: 16, color: .goofGreen).position(x: 60, y: geo.size.height - 80)
                SparkleView(size: 10, color: .goofBlue).position(x: geo.size.width - 30, y: geo.size.height - 100)
            }
        }
    }
    ...
    private var subtitleSection: some View {
        Text(isKorean ? "엉뚱한 일 하나, 오늘도 엉뚱한 하루!" : "One goofy thing, one goofy day!")
            .font(.custom("PyeongChangPeace-Bold", size: 14))
    ...
    private var missionCard: some View {
        VStack(spacing: 16) {
            if todayCompleted >= 1 {
                ...
                VStack(spacing: 12) {
                    Text("🌟")
                        .font(.system(size: 60))
                    Text(isKorean ? "오늘의 엉뚱한 일 완료!" : "Today's Goof Done!")
                        .font(.system(size: 22, weight: .black, design: .rounded))
                    Text(isKorean ? "내일 새로운 엉뚱함으로 만나요!" : "See you tomorrow with new goofiness!")
                ...
                VStack(spacing: 16) {
                    ...
                    Text(task.title(for: settings.language))
                        .font(.custom("PyeongChangPeace-Bold", size: 22))
                ...
                VStack(spacing: 20) {
                    ...
                    Text(isKorean ? "오늘의 엉뚱한 일을\n뽑아보세요!" : "Pick today's\ngoofy mission!")
                        ...
                        .foregroundStyle(Color.goofTextDark.opacity(0.7))
                        .multilineTextAlignment(.center)
        ...
        .frame(minHeight: 280)
        .popArtCard(
            color: todayCompleted >= 1 ? Color.goofPink.opacity(0.4) : (currentTask?.category.cardColor ?? Color.goofSurface),
    ...
    // MARK: - 공유 메뉴
    @MainActor
    private func shareActionMenu(task: GoofyTask) -> some View {
        let renderer = ImageRenderer(content: ShareCardView(task: task, isKorean: isKorean))
        renderer.scale = UIScreen.main.scale
        
        return Group {
            if let image = renderer.uiImage {
                ShareLink(item: Image(uiImage: image), preview: SharePreview("GoofDay", image: Image(uiImage: image))) {
    ...
    private var dateDisplaySection: some View {
        Button(action: resetDailyLimit) {
            ...
            .padding(.vertical, 6)
            .background(Color.goofBlue.opacity(0.1))
            .clipShape(Capsule())
        }
    }
        ...
        VStack(spacing: 12) {
            if todayCompleted < 1 {
                if currentTask == nil {
                    ...
                    .buttonStyle(GoofButtonStyle(color: .goofBlue))
                    .disabled(isSpinning)
                } else {
                    if !isDecided {
                        ...
                        .buttonStyle(GoofButtonStyle(color: .goofRed))
                        ...
                        .buttonStyle(GoofButtonStyle(color: .goofGreen))
                        .disabled(isSpinning)
                    }
                }
            } else {
    ...
    private func completeMission() {
        ...
        let record = GoofyRecord(task: task, status: .completed)
    ...
    private func passMission() {
        guard let task = currentTask else { return }
        let record = GoofyRecord(task: task, status: .passed)
        modelContext.insert(record)
        try? modelContext.save()
    ...
    private func triggerConfetti() {
        confettiParticles = (0..<50).map { _ in
            ConfettiParticle(color: [.goofYellow, .goofBlue, .goofRed, .goofPink, .goofGreen].randomElement()!, x: CGFloat.random(in: 0...400), delay: Double.random(in: 0...0.5))
        }
        showConfetti = true
        DispatchQueue.main.asyncAfter(deadline: .now() + 2.5) {
            showConfetti = false
    ...
    private func resetDailyLimit() {
        ...
        if let records = try? modelContext.fetch(FetchDescriptor<GoofyRecord>(predicate: predicate)) {
